...
if (!defined('MAP_ROOT')) define('MAP_ROOT', WWW_ROOT . DIRECTORY_SEPARATOR . 'cfg' . DIRECTORY_SEPARATOR);
if (!defined('TEMPLATE_ROOT')) define('TEMPLATE_ROOT', WWW_ROOT . DIRECTORY_SEPARATOR . 'ui' . DIRECTORY_SEPARATOR);
...
class TestData {
    ...
    public static function renderHelper($candidate_tmpl) {
        ...
        ICT_View::setMapDir(MAP_ROOT);
        ICT_View::setTemplateDir(TEMPLATE_ROOT);
        ICT_View::setProductName($templateInfo["product"]);
        ICT_View::renderTestPage($templateInfo["id"], $testData);
    }

    public static function parseTemplate($subPath){
        $split = explode("/", $subPath);
        $product = $split[0];
        if(count($split) > 0){
            $product = array_shift($split);
            $ns = array_shift($split);
            $id = $ns . ":" . join("/", $split);
            return array(
                "product"=>$product,
                "ns"=>$ns,
            ...
            var_dump("Maybe your url is not correct, check now!");
            exit;
        }
